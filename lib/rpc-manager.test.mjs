...
test("non-vision models receive attached images as file paths", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const promptSource = source.slice(source.indexOf("case \"prompt\":"), source.indexOf("case \"abort\":"));

  assert.match(promptSource, /modelSupportsImages\(model\?\.input\)/);
  assert.match(promptSource, /imagesToTempFiles\(promptImages\)/);
  assert.match(promptSource, /effectiveImages = undefined/);
});
